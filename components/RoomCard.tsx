...
const RoomCard: React.FC<RoomCardProps> = ({ room, onClick }) => {
  ...
  const idDisplay = useMemo(() => {
     ...
     if (!isSpecial) return <span className="text-slate-400">ID: {host.customId || host.id}</span>;
     
     return (
        <span 
          className="flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-black shadow-lg"
