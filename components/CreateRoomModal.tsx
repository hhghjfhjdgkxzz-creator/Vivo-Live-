    ...
    if (file) {
      ...
      reader.onload = async (event) => {
        if (event.target?.result) {
          ...
          // Smaller max dimensions to avoid document size issues
      ...
      >
        ...
        <div className="space-y-5">
           ...
           <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                 ...
                 <label className="block w-full aspect-square rounded-2xl border-2 border-dashed border-white/10 hover:border-amber-500/50 transition-colors cursor-pointer relative overflow-hidden bg-slate-900 group">
                    {isProcessing ? (
                       <div className="w-full h-full flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-500"></div></div>
                    ) : thumbnail ? (
                       <>
              ...
              <div>
                 ...
                 <label className="block w-full aspect-square rounded-2xl border-2 border-dashed border-white/10 hover:border-purple-500/50 transition-colors cursor-pointer relative overflow-hidden bg-slate-900 group">
                    {isProcessing ? (
                       <div className="w-full h-full flex items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-purple-500"></div></div>
                    ) : background ? (
                       <>
           ...
           <button 
              ...
              className="w-full bg-gradient-to-r from-amber-500 to-orange-600 hover:from-amber-600 hover:to-orange-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-orange-900/30 flex items-center justify-center gap-2 mt-4 active:scale-95 transition-transform disabled:opacity-50"
           >
              {isProcessing ? 'جاري معالجة الصور...' : <><Mic size={20} fill="currentColor" /> ابدأ البث الآن</>}
           </button>
        </div>
      </motion.div>
    </div>
